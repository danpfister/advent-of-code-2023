...
def map_source_to_destination_block(block, mappings): # TODO
...
def parse_mappings(inputdata):
    ...
    return mappings # parsed as (destination, source, end (non-inclusive))
...
def part1and2(inputdata):
    ...
    for mapping in mappings:
        next_stack = []
        while (len(unprocessed_stack) != 0): # loop until all blocks have gone through mapping
            ...
            if unprocessed_block is not None :
